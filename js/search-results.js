...
let lista = '<div class="flex aling-items-center border-input-form put-all-middle-padding middle-margins-vertical generic-border-radius normal-gap flex-direction-column-1024px">';
for (const sear of searchResultsData.results) {
  lista += `<div id="${sear.hotelId}">
                <div">
                  <img src="${sear.imagenUrl}" alt="${sear.name}" class="total-max-width big-max-width generic-border-radius display-block aspect-ratio-square" />
                </div>
                <div class="flex justify-content-space-between total-width position-relative">
                  <div class="flex flex-direction-column big-midle-max-width">
                    ...
                    <p class="generic-color-inputs-text quit-margin-top line-heigh-generic text-generic-size">${sear.description}</p>
                  ...
                  <div class="flex flex-direction-column justify-content-space-between align-items-end justify-content-end">${sear.badge ? (sear.procentaje === 15 ? `<span class="background-color-red text-color-white put-all-little-padding generic-border-radius position-absolute top-0">${sear.badge}</span>` : `<span class="background-color-orange text-color-white put-all-little-padding generic-border-radius position-absolute top-0">${sear.badge}</span>`) : ""}
